...
# Build tinc
# Install tinc 1.1pre16 dependencies as stated in debian:experimental
...
curl http://tinc-vpn.org/packages/tinc-1.1pre16.tar.gz | tar xzC /tmp && \
cd /tmp/tinc-1.1pre16 && ./configure --prefix=/usr --sysconfdir=/etc --localstatedir=/var && make && make install && \
